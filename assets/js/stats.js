...
function worldDailyStats(data){
    ...
    let deceased = [];
    data.forEach((row,i)=>{
            totalConfirmed.push({ x: new Date(`${row.reportDate}`) , y: parseInt(row.totalConfirmed)})
            dailyConfirmed.push({ x: new Date(`${row.reportDate}`) , y: parseInt(row.deltaConfirmed) })
            deceased.push({ x: new Date(`${row.reportDate}`) , y: parseInt(row.deaths.total) })
    });
...
function indianDailyStats(data){
    ...
    data.forEach((row,i)=>{
        ...
        totalConfirmed.push({ x: new Date(`${row.date} 2020`) , y: parseInt(row.totalconfirmed)})
        dailyConfirmed.push({ x: new Date(`${row.date} 2020`) , y: parseInt(row.dailyconfirmed) })
        dailyDeceased.push({ x: new Date(`${row.date} 2020`) , y: parseInt(row.dailydeceased) })
        totalDeceased.push({ x: new Date(`${row.date} 2020`) , y: deaths })
        dailyRecovered.push({ x: new Date(`${row.date} 2020`) , y: parseInt(row.dailyrecovered) })
    });
